...
class PropertySchedule(generic.ListView):
    ...
    def get(self, request, date=0, *args, **kwargs):
        ...
        prev_shifts = Shift.objects.order_by("date").filter(date__gte=first_of_month).filter(date__lt=today).prefetch_related('jobs_in_shift').prefetch_related('jobs_in_shift__job_location')
        remaining_shifts = Shift.objects.order_by("date").filter(date__gte=today).filter(date__lte=end_of_month).prefetch_related('jobs_in_shift').prefetch_related('jobs_in_shift__job_location')
